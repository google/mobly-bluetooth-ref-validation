...
"""Example controller configurations for Bluetooth reference device."""
...
class ConfigError(Exception):
  """The controller configs encounter error."""
...
def from_dicts(configs: Sequence[dict[str, Any]]) -> list[DeviceConfig]:
  ...
  for config in configs:
    logging.debug('Parsing config: %s', config)
...
class DeviceConfig:
  """Provides configs and default values for Mobly controller.

  Attributes:
    serial_port: The serial port of the dev board connected to the Mobly host.
    bluetooth_address: The Bluetooth MAC address of the dev board connected
      to the Mobly host.
  ...
  # The dimensions can then be used to filter devices in the test.
  #
  # Example testbed:
  #   ```config.yaml
  #   BtBoardDevice:
